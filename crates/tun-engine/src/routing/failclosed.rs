...
//! IP, and the DoH resolver IPs, as an RAII guard. Its production caller is the
//! block-until-connected connect gate — held across a covered auto-connect start
//! so a failed connect leaves the host blocked, not leaked. OS specifics live in
//! the platform submodules; this facade is `#[cfg]`-free for callers.
