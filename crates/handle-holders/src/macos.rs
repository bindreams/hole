...
//!
//! `-F pc` emits one field per line, each prefixed with a type
//! character: `p<pid>`, `c<command>`. Records are grouped per process.
//! The filename argument scopes the query so every matched process
//! holds our file.
