...
fn redirect_does_not_loop(#[fixture(temp_dir)] dir: &Path) {
    ...
    assert!(
        !result
            .events
            .iter()
            .any(|e| e.layer == "stderr" && e.target.starts_with("hole::plugin")),
        "stderr layer should not receive plugin events"
    );
}

#[skuld::test]
fn redirect_reformats_go_log_lines_as_plugin_events(#[fixture(temp_dir)] dir: &Path) {
    let result = run_child("plugin_log_reformat", dir);
    // The Go-format line should be parsed and emitted under hole::plugin.
    let plugin_events: Vec<&CapturedEvent> = result.events.iter().filter(|e| e.target == "hole::plugin").collect();
    assert!(
        plugin_events
            .iter()
            .any(|e| e.message.contains("test-plugin-msg") && e.level == "WARN"),
        "expected hole::plugin WARN event with 'test-plugin-msg', got: {:?}",
        result.events,
    );
    // The parsed line must NOT appear under the generic relay target.
    assert!(
        !result
            .events
            .iter()
            .any(|e| e.target == "hole::stderr_relay" && e.message.contains("test-plugin-msg")),
        "parsed line should not appear under hole::stderr_relay: {:?}",
        result.events,
    );
    // Go-format line on stdout should also be parsed under hole::plugin.
    assert!(
        plugin_events
            .iter()
            .any(|e| e.message.contains("test-stdout-plugin-msg") && e.level == "ERROR"),
        "expected hole::plugin ERROR event from stdout with 'test-stdout-plugin-msg', got: {:?}",
        result.events,
    );
    assert!(
        !result
            .events
            .iter()
            .any(|e| e.target == "hole::stdout_relay" && e.message.contains("test-stdout-plugin-msg")),
        "parsed stdout line should not appear under hole::stdout_relay: {:?}",
        result.events,
    );
    // The plain line should still pass through as a generic relay event.
    assert!(
        result
            .events
            .iter()
            .any(|e| e.target == "hole::stderr_relay" && e.message.contains("plain-non-go-line")),
        "plain line should appear under hole::stderr_relay: {:?}",
        result.events,
    );
}
...
// Tests: plugin log parsing ===========================================================================================

#[skuld::test]
fn parse_plugin_log_warning_level() {
    let result = super::try_parse_plugin_log_line("2026/04/08 17:48:30 [Warning] V2Ray started");
    let (level, msg) = result.expect("should parse");
    assert_eq!(level, tracing::Level::WARN);
    assert_eq!(msg, "V2Ray started");
}

#[skuld::test]
fn parse_plugin_log_no_level_tag() {
    let result = super::try_parse_plugin_log_line("2026/04/08 17:48:30 A unified platform");
    let (level, msg) = result.expect("should parse");
    assert_eq!(level, tracing::Level::INFO);
    assert_eq!(msg, "A unified platform");
}

#[skuld::test]
fn parse_plugin_log_error_level() {
    let result = super::try_parse_plugin_log_line("2026/04/08 17:48:30 [Error] connection failed");
    let (level, msg) = result.expect("should parse");
...
#[skuld::test]
fn parse_plugin_log_info_level() {
    let result = super::try_parse_plugin_log_line("2026/04/08 17:48:30 [Info] server listening");
    let (level, msg) = result.expect("should parse");
    assert_eq!(level, tracing::Level::INFO);
    assert_eq!(msg, "server listening");
}

#[skuld::test]
fn parse_plugin_log_debug_level() {
    let result = super::try_parse_plugin_log_line("2026/04/08 17:48:30 [Debug] verbose output");
    let (level, msg) = result.expect("should parse");
    assert_eq!(level, tracing::Level::DEBUG);
    assert_eq!(msg, "verbose output");
}

#[skuld::test]
fn parse_plugin_log_unknown_level() {
    let result = super::try_parse_plugin_log_line("2026/04/08 17:48:30 [Unknown] something odd");
    let (level, msg) = result.expect("should parse");
    assert_eq!(level, tracing::Level::WARN);
...
#[skuld::test]
fn parse_plugin_log_with_session_id() {
    let result =
        super::try_parse_plugin_log_line("2026/04/08 17:48:31 [Warning] [2150376412] app/dispatcher: default route");
    let (level, msg) = result.expect("should parse");
    assert_eq!(level, tracing::Level::WARN);
    assert_eq!(msg, "[2150376412] app/dispatcher: default route");
}

#[skuld::test]
fn parse_non_go_log_returns_none() {
    assert!(super::try_parse_plugin_log_line("not a go log line").is_none());
}

#[skuld::test]
fn parse_tracing_format_returns_none() {
    assert!(super::try_parse_plugin_log_line("2026-04-08T15:48:30Z  INFO something").is_none());
}

#[skuld::test]
fn parse_empty_message_after_timestamp() {
    assert!(super::try_parse_plugin_log_line("2026/04/08 17:48:30 ").is_none());
}

#[skuld::test]
fn parse_empty_message_after_tag() {
    assert!(super::try_parse_plugin_log_line("2026/04/08 17:48:30 [Warning] ").is_none());
}
