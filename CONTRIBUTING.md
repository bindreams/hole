...
1. **If the failure is `Access is denied (os error 5)` on spawn**, grep the log for `file-lock holder`. The `diagnostics::spawn::spawn_with_diagnostics` wrapper (see bindreams/hole#208) enumerates processes holding `hole.exe` on spawn failures. Expect `MsMpEng.exe` (Windows Defender) as the typical culprit. If no holders appear but the spawn still fails, Defender is PPL-protected and our non-`SeDebugPrivilege` enumeration skipped it — the `info!` line "file-lock holder enumeration skipped PIDs we couldn't open" is the tell.
