...
**V2ray-plugin / v2ray-core logs.** The v2ray-plugin Go binary and the v2ray-core library it embeds write to stdout/stderr using Go's standard `log` package (`YYYY/MM/DD HH:MM:SS` timestamp). v2ray-core also prepends a bracketed severity tag from its protobuf `Severity` enum (`[Error]`, `[Warning]`, `[Info]`, `[Debug]`, `[Unknown]`). The relay emitters in [`crates/common/src/logging.rs`](crates/common/src/logging.rs) detect this Go log format, strip the Go timestamp, map the severity tag to a tracing level, and re-emit the message under target `hole::plugin`. Lines that don't match the Go format (from other child processes or non-standard output) pass through as `hole::stderr_relay` / `hole::stdout_relay` at INFO, as before.
